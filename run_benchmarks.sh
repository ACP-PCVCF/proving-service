...
run_benchmarks() {
    ...
    timeout $TIMEOUT cargo test bench_composition $FEATURES -- --ignored --nocapture || echo "TIMEOUT or FAILED: bench_composition"

    echo "[2/3] Proof aggregation"
    timeout $TIMEOUT cargo test bench_proofaggregation $FEATURES -- --ignored --nocapture || echo "TIMEOUT or FAILED: bench_proofaggregation"

    echo "[3/3] Tree aggregation"
    timeout $TIMEOUT cargo test bench_tree_aggregation $FEATURES -- --ignored --nocapture || echo "TIMEOUT or FAILED: bench_tree_aggregation"
}
