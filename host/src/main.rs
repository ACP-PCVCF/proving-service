...
async fn handle_kafka_message(shipments_json: &str) -> Option<ProofResponse> {
    ...
    let mut proofing_document: ProofingDocument = match serde_json::from_str(shipments_json) {
        Ok(s) => s,
        Err(e) => {
    ...
    let serialized_image_id_opt = bincode
        ::serialize(&image_id_opt)
        .expect("Failed to serialize image_id_opt");
    let serialized_journal_opt = bincode
        ::serialize(&journal_opt)
        .expect("Failed to serialize journal_opt");

    proofing_document.proof = None;
