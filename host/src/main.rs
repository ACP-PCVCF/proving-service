...
mod tests {
    ...
    use crate::proofing_document::*;
    use crate::hoc_toc_data::*;
    ...
    async fn smoke_test_with_realistic_shipments_json() {
        ...
        // Call kafka handler
        let resp: ProofResponse = handle_kafka_message(shipments_json)
            .await
        ...
        assert!(
            resp.journal_output.is_finite(),
            "journal_output must be numeric"
        );
        assert!(!resp.image_id.is_empty(), "image_id must be present");
    }

    #[test]
    fn import_test() -> Result<(), Box<dyn std::error::Error>> {
        let file_path = "src/json_file_test.json";
        let json_content = fs::read_to_string(file_path)?;

        let document: ProofingDocument = serde_json::from_str(&json_content)?;

        println!(
            "{}", document.hocData[0].co2eIntensityTTW
        );
